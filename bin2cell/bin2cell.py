...
def stardist(
    ...
    **kwargs,
    ...
    img = load_image(
        image_path, gray=(stardist_model == "2D_versatile_fluo"), dtype=np.float16
    )
    img = normalize(img)
    ...
    *,
    ...
    if load_images:
        tissue_positions_file = (
            ...
            else path / "spatial/tissue_positions.parquet"
        ...
        if files["tissue_positions_file"].name.endswith(".csv"):
            positions = pd.read_csv(
                ...
                header=0
                if tissue_positions_file.name == "tissue_positions.csv"
                else None,
                index_col=0,
            )
...
def destripe_counts(
    adata, counts_key="n_counts", adjusted_counts_key="n_counts_adjusted"
):
    """
    ...
    bin_scaling = scipy.sparse.diags(scaling_factors)
...
def destripe(
    ...
    if adjust_counts:
        destripe_counts(
            adata, counts_key=counts_key, adjusted_counts_key=adjusted_counts_key
        )
...
def grid_image(adata, val, log1p=False, mpp=2, sigma=None, save_path=None):
    ...
    img = np.zeros(
        (
            adata.uns["bin2cell"]["array_check"]["row"]["max"] + 1,
...
def insert_labels(
    ...
    adata.obs.loc[mask, labels_key] = np.asarray(
...
def expand_labels(
    ...
    adata.obs.loc[
        adata.obs_names[clear_query_inds], expanded_labels_key
    ] = clear_query_labels
    ...
    # insert calls into object
    adata.obs.loc[
        adata.obs_names[same_query_inds], expanded_labels_key
    ] = same_query_labels
    ...
    if np.sum(ambiguous_mask) > 0:
        ...
        if subset_pca:
            ...
            smol = np.unique(
                np.concatenate(
                    [
                        hits[ambiguous_mask, 0],
                        hits[ambiguous_mask, 1],
                        ambiguous_query_inds,
                    ]
                )
            )
        ...
        hit0eucl = np.linalg.norm(
            pca[hits[ambiguous_mask, 0], :] - pca[ambiguous_query_inds, :], axis=1
        )
        hit1eucl = np.linalg.norm(
            pca[hits[ambiguous_mask, 1], :] - pca[ambiguous_query_inds, :], axis=1
        )
        ...
        ambiguous_query_labels[hit0eucl < hit1eucl] = calls[ambiguous_mask, 0][
            hit0eucl < hit1eucl
        ]
        ambiguous_query_labels[hit0eucl > hit1eucl] = calls[ambiguous_mask, 1][
            hit0eucl > hit1eucl
        ]
        # insert calls into object
        adata.obs.loc[
            adata.obs_names[ambiguous_query_inds], expanded_labels_key
        ] = ambiguous_query_labels
...
def salvage_secondary_labels(
    ...
    secondary_to_take = np.array(
        list(set(secondary[secondary_label]).difference(set(primary[secondary_label])))
    )
...
def bin_to_cell(
    ...
    cell_adata.obs["array_row"] = row_means.dot(cell_to_bin).dot(
        adata.obs["array_row"].values
    )
    cell_adata.obs["array_col"] = row_means.dot(cell_to_bin).dot(
    ...
    for spatial_key in spatial_keys:
        cell_adata.obsm[spatial_key] = row_means.dot(cell_to_bin).dot(
    ...
    library = list(adata.uns["spatial"].keys())[0]
    cell_adata.uns["spatial"][library]["scalefactors"][
    ...
    if labels_key + "_source" in adata.obs.columns:
        ...
        mapping = (
            adata.obs[[labels_key, labels_key + "_source"]]
            .drop_duplicates()
            .astype(str)
            .set_index(labels_key)
            .to_dict()[labels_key + "_source"]
        )
        # translate the labels from the cell object
        cell_adata.obs[labels_key + "_source"] = [
            mapping[i] for i in cell_adata.obs_names
        ]
    return cell_adata
