...
from src.destriping.simulation.segmentation_mask_generator import (
    SegmentationMaskGenerator,
    SegmentationMask,
)
from src.destriping.simulation.poisson_count_generator import SpatialCountGenerator
from src.destriping.simulation.stripe_factors_generator import (
    WeibullStripeFactorsGeneratorConstrained,
    plot_stripes,
    statistics_stripes,
)
from src.destriping.simulation.cell_expression_generator import (
    ExpectedConcentrationGenerator,
)
import numpy as np
...
generator_map = {"weibull_constrained": WeibullStripeFactorsGeneratorConstrained}


def generator_cls_from_str(
    generator_name: str,
) -> type[WeibullStripeFactorsGeneratorConstrained]:
...
def generate_spatial_data(
    ...
    if segmentation_mask_path is not None:
        ...
        generator = SegmentationMaskGenerator(
            shape=(n_rows, n_cols),
    ...
    if expected_concentration_gene_profile_path is not None:
        expected_concentration_gene_profile_ = anndata.read_h5ad(
            expected_concentration_gene_profile_path
        )
    else:
        ref = sc.read(sc_ref_path)
        expression_generator = Sc_expression_generator(
            n_cells=n_cells, n_genes=n_genes, reference=ref, random_seed=random_seed
        )
        expr_data = expression_generator.generate()
        expected_concentration_gene_profile_generator = ExpectedConcentrationGenerator(
            expr_data.X,
            avg_tot_counts_per_bin,
            sigma_tot_counts_per_bin_across_cells,
            random_seed,
        )
        expected_concentration_gene_profile_X = (
            expected_concentration_gene_profile_generator.generate()
        )
    ...
    if isinstance(stripes_generator_params, (tuple, list)):
        horizontal_stripes_params = (
            stripes_generator_params[0]
            if stripes_generator_params[0] is not None
            else {}
        )
        vertical_stripes_params = (
            stripes_generator_params[1]
            if stripes_generator_params[1] is not None
            else {}
        )
    else:
        horizontal_stripes_params = vertical_stripes_params = (
            stripes_generator_params if stripes_generator_params is not None else {}
        )
    ...
    horizontal_stripes = horizontal_generator_cls(
        n_rows, **horizontal_stripes_params, random_seed=random_seed
    ).generate()
    vertical_stripes = vertical_generator_cls(
        n_cols, **vertical_stripes_params, random_seed=random_seed * 2
    ).generate()
