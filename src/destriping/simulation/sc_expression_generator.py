...
class Sc_expression_generator():
    def __init__(self, n_cells, n_genes, reference: anndata.AnnData, random_seed=None):
        ...
        if n_cells > self.reference.shape[0]:
            raise ValueError(f"n_cells ({n_cells}) cannot be greater than the number of cells in the reference ({self.reference.shape[0]})")
        if n_genes > self.reference.shape[1]:
            raise ValueError(f"n_genes ({n_genes}) cannot be greater than the number of genes in the reference ({self.reference.shape[1]})")
    ...
    def generate(self) -> anndata.AnnData: 
        ...
        cell_indices = self.generator.choice(self.reference.shape[0], size=self.n_cells, replace=False)
        
        # Identify highly variable genes
        sc.pp.highly_variable_genes(self.reference, n_top_genes=self.n_genes, inplace=True, flavor='seurat_v3')
