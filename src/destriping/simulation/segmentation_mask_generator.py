...
class SegmentationMask:
    ...
    def all_lanes_occupied(self) -> bool:
        ...
        all_rows_occupied = len(nucleic_bin_df["row_idx"].value_counts()) == len(
            all_bin_df["row_idx"].value_counts()
        )
        all_cols_occupied = len(nucleic_bin_df["col_idx"].value_counts()) == len(
            all_bin_df["col_idx"].value_counts()
        )
...
class SegmentationMaskGenerator:
    def __init__(
        self,
        shape: tuple[int, int],
        occupational_density: float,
        min_cell_size: int,
        max_cell_size: int,
        n_trials_all_lanes_occupied: int = 3,
        n_trials_shape: int = 50,
    ...
    def generate(self) -> SegmentationMask:
        ...
        min_diam, max_diam = get_range_diameter_from_range_size(
        ...
        if not (all_lanes_occupied):
            warnings.warn(
                f"After {self.n_trials_all_lanes_occupied} trials, did not manage to occupy all lanes."
            )
