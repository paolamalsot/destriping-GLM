...
def statistics_stripes(stripe_factors: NDArray[np.float64]):
    ...
    return {"mean": mean, "std": std, "mean/std": mean / std, "sum": sum}
...
class WeibullStripeFactorsGenerator(StripeFactorsGenerator):
    ...
    def generate(self) -> StripeFactors:
        stripe_factors = stats.weibull_min.rvs(
            self.shape, loc=self.loc, scale=self.scale, size=self.n_lanes
        )
        # clip
...
class WeibullStripeFactorsGeneratorConstrained(
    WeibullStripeFactorsGenerator
):  # constrained to sum to n_lanes
    def __init__(
        self,
        n_lanes: int,
        loc: float,
