...
def poisson_generator_with_csr_mu(mu, generator):
    ...
    poisson_counts_matrix = csr_matrix(
        (poisson_counts, mu.indices, mu.indptr), shape=mu.shape
    )
...
def negative_binomial_generator_with_csr_mu(mu, generator, dispersion):
    ...
    # create a new csr_matrix with the same shape as mu
    negative_binomial_counts_matrix = csr_matrix(
        (negative_binomial_counts, mu.indices, mu.indptr), shape=mu.shape
    )
...
class NegativeBinomialCountGenerator:
    ...
    def generate(self, expectation_counts_matrix):
        if not isinstance(expectation_counts_matrix, csr_matrix):
            raise ValueError("expectation_counts_matrix must be a csr_matrix")
        return negative_binomial_generator_with_csr_mu(
            expectation_counts_matrix, self.generator, self.dispersion
        )
...
class SpatialCountGenerator:
    def __init__(
        ...
        self.distribution_params = (
            distribution_params if distribution_params is not None else {}
        )
        ...
        self.sigma_tot_counts_per_bin_across_cells = (
            sigma_tot_counts_per_bin_across_cells
        )

        self.horizontal_stripe_factors = horizontal_stripe_factors
        self.vertical_stripe_factors = vertical_stripe_factors

        assert segmentation_mask.shape == (
            len(self.horizontal_stripe_factors),
            len(self.vertical_stripe_factors),
        )
        if (
            segmentation_mask.n_cells
            != self.expected_concentration_gene_profile_.shape[0]
        ):
            raise ValueError(
                "Different number of cells in expected_concentration_gene_profile_ and in segmentation_mask"
            )
    ...
    def generate(self) -> tuple[spatialAdata, AnnData, spatialAdata, spatialAdata]:
        ...
        expected_concentration_gene_profile_ = (
            self.expected_concentration_gene_profile_
        )  # n_cells x n_genes
        ...
        unique_categories, int_cell_id = np.unique(
            self.segmentation_mask.nuclei_bin_df["cell_id"], return_inverse=True
        )
        bin_df.loc[nulei_indices, "reference_index"] = (
            self.expected_concentration_gene_profile_.obs["reference_index"]
            .iloc[int_cell_id]
            .values
        ...
        expectation_counts_matrix_unstriped = lil_matrix(
            (n_obs, n_genes), dtype=np.float32
        )
        # https://stackoverflow.com/a/20344429
        expectation_counts_matrix_unstriped[nulei_indices] = indexing_csr(
            expected_concentration_gene_profile_.X, int_cell_id, 100000
        )
        expectation_counts_matrix_unstriped = (
            expectation_counts_matrix_unstriped.tocsr()
        )

        expectation_counts_matrix = expectation_counts_matrix_unstriped.multiply(
            self.horizontal_stripe_factors[bin_df.row_idx].reshape(-1, 1)
        ...
        bin_df[
            "n_counts_expected_without_stripes"
        ] = expectation_counts_matrix_unstriped.sum(-1)
        bin_df["stripe_factor"] = (
            self.horizontal_stripe_factors[bin_df.row_idx].toarray()
            * self.vertical_stripe_factors[bin_df.col_idx].toarray()
        )

        # generated_counts_matrix = self.generator.poisson(expectation_counts_matrix)
        expectation_counts_matrix = csr_matrix(
            expectation_counts_matrix, dtype=np.float32
        )
        count_generator = get_count_generator(
            self.distribution, self.distribution_params, self.random_seed
        )
        generated_counts_matrix = convert_to_32_bit(
            count_generator.generate(expectation_counts_matrix)
        )
        array_coordinates = bin_df.loc[
            :, ["row_idx", "col_idx"]
        ].values  # top down, left to right
        bin_df.index = bin_df.index.astype("str")
        var = self.expected_concentration_gene_profile_.var
        spatialdata = make_sdata(
            array_coordinates, generated_counts_matrix, obs=bin_df, var=var
        )
        ...
        obs = pd.DataFrame(
            data={
                "cell_id": unique_categories,
                "reference_index": self.expected_concentration_gene_profile_.obs[
                    "reference_index"
                ].values,
            }
        )
        ...
        cell_gene_expression_profile.obs.index = cell_gene_expression_profile.obs[
            "cell_id"
        ].values

        return (
            spatialdata,
            cell_gene_expression_profile,
            expected_sdata,
