...
def make_sol(h_row, h_values, w_col, w_values, data_row_indices, data_col_indices, data_index, fitted_c):
    h = make_stripes_serie(h_row, h_values)
    w = make_stripes_serie(w_col, w_values)
    h_per_bin = h.reindex(data_row_indices) #used instead of .loc to avoid error in case of missing indices
