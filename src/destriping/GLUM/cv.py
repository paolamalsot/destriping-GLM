...
def _reestimate_c_and_predict(glm, X_test, y_test, offset_test):
    """Re-estimate c on val data given fitted h,w, then predict.

    Extracts h, w from the fitted model's coefficients, computes
    c_p = sum(y) / sum(h_i * w_j) per nucleus on the val fold,
    then returns mu = c_p * h_i * w_j.
    """
    from src.destriping.GLUM.fit import glum_coef_to_hwc, _parse_feature_names

    logger.debug("Reestimating c on val fold (%d obs)", len(y_test))

    coef = glm.coef_
    intercept = glm.intercept_
    feature_names = glm.feature_names_

    # Use cached feature-name parsing (shared with extract_coef_specific)
    var_indices = _parse_feature_names(tuple(feature_names))

    # The dropped level is NOT in feature_names — we need it from X_test
    dropped_levels_dict = {}
    for col in ["i", "j", "p"]:
        if col in X_test.columns and hasattr(X_test[col], "cat"):
            all_levels = X_test[col].cat.categories.tolist()
            present_levels = set(var_indices.get(col, ([], []))[1])
            dropped = [l for l in all_levels if str(l) not in present_levels]
            dropped_levels_dict[col] = dropped[0] if dropped else all_levels[0]
    ...
    sum_hw = np.bincount(p_codes, weights=hw, minlength=n_p)

    c_per_p = np.where(sum_hw > 0, np.maximum(sum_y, 1e-300) / sum_hw, 1.0)
    mu = c_per_p[p_codes] * hw
    return mu
...
def _run_one_split_df(
    *,
    ...
    for param_i, params in enumerate(param_grid_sk):
        ...
        if reestimate_c_on_val:
            mu = _reestimate_c_and_predict(glm, X_test, y_test, offset_test)
        else:
...
def glm_cv(
    ...
    *,
    ...
    if parallel:
        split_dfs = Parallel(n_jobs=n_jobs, prefer="threads")(
