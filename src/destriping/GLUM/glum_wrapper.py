...
from src.destriping.GLUM.init import (
    get_ratio_init_sol,
    get_ones_init_sol,
...
def build_poisson_sol_from_stripings(sol, spatialdata, cell_id_label):
    ...
    sol_new.c.index = sol_new.c.index.to_series().apply(
        lambda x: x.replace("id_", "", 1)
    )
    sol_new_c = (
...
class GlumWrapper:
    ...
    def fit(self):
        ...
        if self.regressorCV or self.sklearnCV:
            ...
            supp_kwargs = {"cv": cv, "P2": P2, "P1": P1, "alphas": alphas}
        else:
            supp_kwargs = {}
    ...
    def get_sol(self):
        poisson_sol = build_poisson_sol_from_stripings(
            self.sol_, self.data, self.id_label
        )
