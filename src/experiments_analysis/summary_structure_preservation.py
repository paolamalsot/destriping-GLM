...
def difference_between_smoothed_curves(df, comp_keys, reference_keys, save_dir, k=100):
    ...
    group_cols = ["operation_name", "ref", "comp", "metric"]
    new = (
        results_df.groupby(group_cols)
        .agg(
            difference=("difference", lambda x: x.sum(skipna=False)),
            lane_name=("lane_name", "sum"),
        )
        .reset_index()
    )
    new["axis"] = "global"
...
def striping_intensity_cyto(matrix, cyto_select, axis, normalized=False):
    ...
    """
    if not np.all(np.isfinite(matrix[cyto_select])):
        return np.nan
    if axis == 1:
