...
from src.experiments_analysis.analysis_utils import get_destriped_tot_counts, get_index_gt_destriped
...
def dataset_folder_from_folder_result(folder_results):
    ...
    folder_results_in = [folder for folder in folder_results.iterdir() if folder.is_dir()][0]
    folder_results_in = [folder for folder in folder_results_in.iterdir() if (folder.is_dir() and ("df_results.csv" in [f.name for f in folder.iterdir()]))][0]
...
color_dict = {"original": "yellow", "GT": "blue", "b2c": "red", "neighbour_loss_no_reg_within_nucl": "green", "neighbour_loss_best":"darkgreen", "neighbour_loss_no_reg_all": "chartreuse", "neighbour_loss_worst":"mediumspringgreen", "init_quantiles_nucl": "brown", "init_quantiles": "orange", "init_median_ratio": "khaki"}
to_plot_simulation = [
    "original",
    "GT",
    "b2c",
...
to_plot_real_data.append("neighbour_loss_no_reg_all")  # GT is not available for real data

def make_plots_global_structure(df, to_plot, save_dir: P, k = 100, color_dict_ = None, style_dict = None):
...
def difference_between_smoothed_curves(df, comp_keys, reference_keys, save_dir, k = 100, cosine_dist = False):
    ...
    for axis, lane_name in zip([1, 0], ["rows", "columns"]):
        for operation_name, operation in operation_dict.items():
            ...
            for ref in reference_keys:
                ...
                with warn_with_prefix(f"Calculating smoothed line for {ref=}, {operation_name} and {axis=}: "):
                    smoothed_line_ref = smoothed_line_from_key(df, index, operation_name, k, axis)
                ...
                for comp in comp_keys:
                    ...
                    with warn_with_prefix(f"Calculating smoothed line for {comp=}, {operation_name} and {axis=}: "):
                        smoothed_line_comp = smoothed_line_from_key(df, index_comp, operation_name, k, axis)
                    ...
                    if cosine_dist:
                        ...
                        difference = np.linalg.norm(smoothed_line_ref - smoothed_line_comp)
                    result_dict = {"axis": axis, "lane_name": lane_name, "operation_name": operation_name, "ref": ref, "comp": comp, "difference": difference}
    ...
    for operation_name, df in results_df.groupby("operation_name"):
        g = sns.catplot(kind="strip", x = "comp", y = "difference", row = "ref", col = "axis", hue = "comp", data = df)
        for ax in g.axes.flat:
...
def striping_intensity(matrix, axis, normalized = False):
    ...
    I = (np.diff(np.nansum(matrix, other_axis, keepdims = True), axis=axis)**2).sum()**0.5
    if normalized:
...
def striping_intensity_cyto(matrix, cyto_select, axis, normalized = False):
    ...
    diffs_cyto_only = np.ma.array(data =np.diff(matrix, axis=axis),mask =~cyto_connection)
...
def striping_intensity_statistics(
    df, output_folder, path_dataset, normalized=False
):
...
def striping_intensity_cyto_statistics(df, output_folder, path_dataset, nucl_label, normalized = False):
...
def plots_striping_intensity_statistics(df_striping_intensity, output_folder):
    ...
    output_folder.mkdir(parents=True, exist_ok=True)
    df = pd.wide_to_long(df_striping_intensity, stubnames = ["striping_intensity"], i = "name", j = "lane", sep = "_", suffix='\\w+').reset_index()
    g = sns.catplot(col = "lane", x = "name", y = "striping_intensity", kind = "strip", data = df, log_scale = True)
...
def save_n_counts_matrices(df_results, index_oi_dict, output_folder):
    ...
    for name, index in index_oi_series.items():
           ...
           new_df_list.append({"name": name, "index_in_df_results": index, "path_destriped_n_counts_matrix": str(output_folder / f"matrix_{index}.npy")})
