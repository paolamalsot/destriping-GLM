...
from src.experiments_analysis.analysis_utils import build_df_from_sdata, get_cell_id_label, get_dataset_path, get_destriped_tot_counts, get_destriped_tot_counts_from_df, get_df_destriped_data_from_row, load_gt_sol, get_index_gt_destriped
from src.experiments_analysis.glum_collect_runs import collect_runs, process_baselines_folders
...
def load_supp_baselines(dir_):
    ...
    df_supp_baselines = pd.concat([df_supp_baselines.reset_index(drop=True), S_df], axis=1)
...
def check_nan_inf_destriped_data(destriped_summary_df):
    for n_counts_adjusted in [True, False]:
        ...
        destriped_summary_df[colname] = destriped_summary_df.apply(check_nan_inf_value, n_counts_adjusted = n_counts_adjusted, axis = 1)
    return destriped_summary_df


def add_processed_data_paths_lightweight(x: pd.Series):
    data_dicts = get_processed_data_dicts_lightweight(os.path.join(x.run_dir, "destriped_data"))
...
def summarize_destriped_data_sols_from_runs(
    ...
    if df_final["nan_inf_destriped_data__n_counts_adjusted=True"].any() or df_final["nan_inf_destriped_data__n_counts_adjusted=True"].any():
        list_nan = df_final.query("nan_inf_destriped_data__n_counts_adjusted=True or nan_inf_destriped_data__n_counts_adjusted=True")["name"].tolist()
...
def get_processed_data_dicts_lightweight(dir_):
    ...
    for subdir in os.listdir(dir_): #remove self...
        ...
        assert os.path.exists(dict_["destriped_data_path"]), print(f"{dict_['destriped_data_path']} does not exist")
...
def filter_summary_df_qm(destriped_summary_df):
    def select_method(destripe_method_df):
        for method in destripe_method_df["destriping_method"].tolist():
            if pd.isna(method):
                continue
            if ("qm" in method) and not ("sqm" in method): #for compatibility with my previous results
...
def calc_distance_destriped_data_to_gt(df, distance_fun_dict_, gt_destriping_method = "qm"):
    ...
    for dataset_path, df_dataset in df.groupby("dataset_path"):
        ...
        gt_poisson_data_tot_counts_nucl = gt_poisson_data_tot_counts.loc[nuclear_indices]
        ...
        def distance_to_gt(row):
            ...
            for take_n_counts_adjusted in [True, False]:
                ...
                pd.testing.assert_index_equal(destriped_data_tot_counts.index, original_data.obs.index)
                to_repl = inf | na.values
                destriped_data_tot_counts.loc[to_repl] = original_data.obs.loc[to_repl, "n_counts"]
