...
def normalized_euclidian_distance_fun(v_i, v_j):
    ...
    assert v_i.values.ndim == 1
    dist_ = euclidean(v_i[intersect_indices].values, v_j[intersect_indices].values)/np.sqrt(len(intersect_indices)) #euclidean is necessarily 1D
    return dist_
...
distance_fun_dict = {"cosine": cosine_distance_fun,
...
def get_distance_fun(metric_name, distance_fun, param_name):
    if metric_name == "log_euclidian":
        distance_fun_ = lambda v_i, v_j: distance_fun(v_i, v_j, log_offset_dict[param_name])
    else:
