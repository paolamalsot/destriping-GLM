...
def make_spatial_data_from_numpy_2D(data):
    ...
    row_indices = (
        np.broadcast_to(np.arange(nrows).reshape(-1, 1), data.shape)
        .flatten()
        .reshape(-1, 1)
    )
    col_indices = (
        np.broadcast_to(np.arange(ncols).reshape(1, -1), data.shape)
        .flatten()
        .reshape(-1, 1)
    )
...
def make_sdata(coordinates, count_matrix, obs=None, var=None):
    ...
    n_counts = np.array(
        np.sum(count_matrix, axis=-1)
    ).flatten()  # necessary for matrix...
    if obs is None:
    ...
    }  # doesn't have any meaning, just to have a default scalefactor of 1 !
    ...
    coordinates_df_ = coordinates_df.from_records(
        [{"coordinate_id": "array", "img_key": pd.NA, "scalefactor": 1.0}]
    )
    coordinates_df_.df["img_key"] = coordinates_df_.df["img_key"].astype(object)
    img_df_ = img_df()
    spatialdata = spatialAdata(
        adata, library_id=library_id, coordinate_df=coordinates_df_, img_df=img_df_
    )
    return spatialdata
