...
def check_correlation_array_coord_with_img_coord(coords_array, coords_img):
    ...
    logging.info(
        f"Correlation between image coordinates and array coordinates for coord 0 (corr, pval): {coord_0_corr}"
    )
    logging.info(
        f"Correlation between image coordinates and array coordinates for coord 1 (corr, pval): {coord_1_corr}"
    )
...
def reorient_coords(array_coords, img_coords):
    ...
    coord_0_corr, coord_1_corr = check_correlation_array_coord_with_img_coord(
        array_coords, img_coords
    )
