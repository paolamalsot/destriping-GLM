...
def get_qm_fun(dist, dist_params=None):
    ...
    if dist == "poisson":
        ...
        return lambda k, mu, f: quantile_match_nbinom(
            k=k, mu=mu, r=dist_params["r"], f_=f
        )
    else:
        raise NotImplementedError(f"Distribution {dist} is not implemented")


def quantile_match_poisson(
    k: np.array, mu: np.array, f_: np.array, verbose=False
) -> np.array:
    """
...
def quantile_match_nbinom(
    k: np.array, mu: np.array, r: np.array, f_: np.array, verbose=False
) -> np.array:
    """
    ...
    if np.sum(zero_selector) > 0:
        res[zero_selector] = 0
    # retry inf entries via sf/isf: cdf saturates to 1 and ppf(1)=inf, while
    # sf can stay numerically positive and isf returns a finite value.
    inf_selector = np.isinf(res)
    if np.sum(inf_selector) > 0:
        res[inf_selector] = quantile_match_nbinom_via_sf(
            k=np.asarray(k)[inf_selector],
            mu=np.asarray(mu)[inf_selector],
            r=r,
            f_=np.asarray(f_)[inf_selector] if np.ndim(f_) > 0 else f_,
            verbose=verbose,
        )
    return res


def quantile_match_nbinom_via_sf(
    k: np.array, mu: np.array, r: np.array, f_: np.array, verbose=False
) -> np.array:
    """
    Same as quantile_match_nbinom but using sf -> isf instead of cdf -> ppf.
    sf(k) = 1 - cdf(k) = P(X > k); isf(q) = ppf(1 - q).
    When cdf is ~1.0, ppf returns inf; sf stays numerically positive and isf stays finite.
    """
    if type(mu) == np.matrix:
        raise ValueError("quantile match does not accept matrices but arrays")
    ...
    muf = mu * f_
    n, p = convert_n_binom_params(mu=muf, alpha=r)
    sf_val = nbinom.sf(k, n=n, p=p)
    if verbose:
        print(f"{k=}, {muf=},{sf_val=}")
    n, p = convert_n_binom_params(mu=mu, alpha=r)
    res = nbinom.isf(q=sf_val, n=n, p=p)
    zero_selector = (k == 0) & (mu == 0)
    if np.sum(zero_selector) > 0:
        res[zero_selector] = 0
    eps = 1e-3
...
def quantile_match_sparse(
    k, mu, f_, verbose=False, method=quantile_match_poisson
) -> sp.sparray:
    """
    Performs quantile mapping from the poisson dist param mu * f_ to the poisson dist of param mu.
