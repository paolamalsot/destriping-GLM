...
def configuration_dict_to_executor_dict(configuration_dict):
    executor_dict = {key: define_lambda_function(kwargs) for key, kwargs in configuration_dict.items()}
...
def load_default_submitit_executor_dict():
    return load_submitit_executor_dict(path_default_submitit_executor_configuration_dict)