...
def enforce_unique_keys(df, unique_keys):
    if unique_keys:
        ...
        if not duplicates.empty:
            raise ValueError(
                f"Duplicate entries found for keys: {unique_keys}.\n{duplicates}"
            )
...
class UniqueKeyDF:
    def __init__(self, columns, unique_keys=[], dtypes=None):
        ...
        if dtypes is None:
            ...
            self.df = pd.DataFrame(
                {col: pd.Series(dtype=dtype) for col, dtype in zip(columns, dtypes)}
            )
    ...
    def append_dict(self, dict_row):
        ...
        intersection = pd.merge(
            new_df[self.unique_keys], self.df[self.unique_keys], how="inner"
        )
        ...
        if not intersection.empty:
            ...
            self.df = self.df[
                ~self.df[self.unique_keys].isin(keys_intersect).all(axis=1)
            ]
    ...
    def copy(self):
        new_df = UniqueKeyDF(
            columns=self.columns, unique_keys=self.unique_keys, dtypes=self.dtypes
        )
...
class img_df(UniqueKeyDF):
    ...
    def __init__(self):
        super().__init__(
            columns=["img_key", "in_memory", "path", "scalefactor"],
...
class coordinates_df(UniqueKeyDF):
    ...
    def __init__(self):
        super().__init__(
            columns=["coordinate_id", "img_key", "scalefactor"],
            unique_keys=self.unique_keys,
            dtypes=self.dtypes,
        )
