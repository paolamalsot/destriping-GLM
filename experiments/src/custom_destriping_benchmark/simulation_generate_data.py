...
def parse_config(original_root, cfg):
    ...
    if simulation_params["sc_ref_path"] is not None:
        simulation_params["sc_ref_path"] = os.path.join(
            original_root, simulation_params["sc_ref_path"]
        )
    if simulation_params["segmentation_mask_path"] is not None:
        simulation_params["segmentation_mask_path"] = os.path.join(
            original_root, simulation_params["segmentation_mask_path"]
        )
    if (
        "expected_concentration_gene_profile_path" in simulation_params.keys()
        and simulation_params["expected_concentration_gene_profile_path"] is not None
    ):
        simulation_params["expected_concentration_gene_profile_path"] = os.path.join(
            original_root, simulation_params["expected_concentration_gene_profile_path"]
        )
    ...
    ) = spatial_data_generator.generate()
    ...
    gt_sol = Sol(
        pd.Series(spatial_data_generator.horizontal_stripe_factors),
    ...
    expected_sdata_wo_stripes_path = os.path.join(
        main_output_dir, "expected_spatial_data_wo_stripes"
    )
    expected_sdata_wo_stripes.save(expected_sdata_wo_stripes_path)

    cell_gene_expression_path = os.path.join(
        main_output_dir, "cell_gene_expression.h5ad"
    )
