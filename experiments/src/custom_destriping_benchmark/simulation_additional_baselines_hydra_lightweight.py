...
from experiments.src.custom_destriping_benchmark.utils import convert_gen_params_to_qm_params, generating_data_distribution, make_path_rel_root_lambda
...
def parse_config(original_root, cfg):
    ...
    simulation_data_dir = os.path.join(original_root, os.path.dirname(cfg.dataset.path_data))
    bin_data_path = os.path.join(simulation_data_dir, "spatial_data")
    cell_id_label = cfg.dataset.cell_id_label
    
    df_gt_cell_labels = pd.DataFrame(columns = ["model_name", "fitting_args", "destriping_method"])
    ...
    distribution, distribution_params = convert_gen_params_to_qm_params(
        *generating_data_distribution(os.path.join(original_root,cfg.dataset.path_data))
        )
    ...
    for name_dist, distribution_params_ in iterator:
        destriping_calls_ = get_all_destripe_calls(sol, model, data, cell_id_label = cell_id_label, dist = name_dist, dist_params = distribution_params_)
        for name, call in destriping_calls_.items():
            logging.info(name)
            destriped_data = call()
            destriped_data_output_dir = os.path.join(output_dir, "destriped_data", name_dist, name)
            # lightweight change
            save_n_counts_adjusted_counts(destriped_data, destriped_data_output_dir)

            df_gt_poisson_sol_list.append(pd.DataFrame(data = {
                                                            "destriped_data_path": [Path(path_rel_root(destriped_data_output_dir)) / "df.parquet"], 
    ...
    expected_sdata_wo_stripes_path = os.path.join(simulation_data_dir, "expected_spatial_data_wo_stripes")
    data = load_spatialAdata(expected_sdata_wo_stripes_path)
    expected_sdata_wo_stripes_output_dir = os.path.join(output_dir, "expected_sdata_wo_stripes")
    save_n_counts_adjusted_counts(data, expected_sdata_wo_stripes_output_dir)
    
    df_gt_poisson_sol_list.append(pd.DataFrame(data = {
                                                        "destriped_data_path": [Path(path_rel_root(expected_sdata_wo_stripes_output_dir)) / "df.parquet"], 
