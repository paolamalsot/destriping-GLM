...
def generating_data_distribution(path_data):
    ...
    simulation_params = yaml.safe_load(hydra_config_path.read_text())["simulation_params"]
    if "distribution" in simulation_params.keys() and "distribution_params" in simulation_params.keys():
        return simulation_params["distribution"], simulation_params["distribution_params"]
    else: #default
        return "poisson", {}
...
def args_combi_nucl_cyto(sol):
    nucl_args = {"row_factors": sol.h,
                 "col_factors": sol.w,
                 "gene_expression_per_bin": sol.c}
    cyto_args = {"row_factors": sol.h, 
                 "col_factors": sol.w}
...
def get_args_for_method(method, sol, dist = "poisson", dist_params = None):
    if method.__name__ == spatialAdata.destripe_dividing_factors.__name__:
        return {"row_factors": sol.h, 
                 "col_factors": sol.w}
    elif method.__name__ in [spatialAdata.destripe_dividing_factors_qm_tot_counts.__name__]:
        return {"row_factors": sol.h,
                 "col_factors": sol.w,
                 "gene_expression_per_bin": sol.c,
                 "dist": dist,
...
def get_all_destripe_calls(sol, model, data, cell_id_label, dist = "poisson", dist_params = None):
    ...
    methods_list = [spatialAdata.destripe_dividing_factors, 
    ...
    for method in methods_list:
        if c_all:
            ...
            args_nucl = get_args_for_method(nucl_method, sol, dist = dist, dist_params = dist_params)
            args_cyto = get_args_for_method(cyto_method, sol, dist = dist, dist_params = dist_params)

            def apply_after_copy(data_, method_name, args_dict_):
                u = data_.copy()
                return getattr(u, method_name)(**args_dict_)

            args_dict = {"method_cyto": cyto_method,
                         "method_nucl": nucl_method,
                        "nucl_indices": nucl_indices,
                        "args_nucl": args_nucl,
                        "args_cyto": args_cyto
            }
            dict_calls[name_method] = partial(apply_after_copy, data_ = data, method_name = "destripe_combi_nucl_cyto", args_dict_ = args_dict)
