...
from src.destriping.init_params import (
    quantile_init_from_sdata,
    ones_init_from_sdata,
    ratio_init_from_sdata,
)
...
def parse_config(original_root, cfg):
    ...
    if cfg.factors == "ones":
        ...
        h, w = quantile_init_from_sdata(
            data, quant_init=cfg.quant_init, nucl_only=True, cell_id_label=cell_id_label
        )
    elif cfg.factors == "median_ratio":
        h, w = ratio_init_from_sdata(
            data, quantile=cfg.quant_init, cell_id_label=cell_id_label
        )
    else:
